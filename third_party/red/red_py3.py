...
class Red(object):
    ...
    def construct_network(self, gene_set):
        ...
        for _ in range(n):
            ...
            if unordered:
                ...
                if not len(diff):
                    ...
                    for j in range(len(unordered) - 1, np.argmax(diff), -1):
                        i = l[-j][1]
                        order[e_ind].append(i)
                        unordered.remove(i)
                        O[:, i] = O[i, :] = 0
                    ...
                    for j in range(np.argmax(diff) + 1):
                        i = l[j][1]
                        order[s_ind].append(i)
                        unordered.remove(i)
                        O[:, i] = O[i, :] = 0
        ...
        i2o = {g: ord2ind[ordr] for ordr, genes in order.items() for g in
               genes}
        i2g = {i: g for g, i in self._g2i.items()}
